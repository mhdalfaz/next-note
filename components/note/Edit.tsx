...
import { changeFormatDate, getCurrentDate } from "@/utils/helpers";
...
const Edit = ({ note }: { note: Note }) => {
  ...
  const [data, setData] = useState(() => {
    const date = changeFormatDate(note.date);
    return {
      ...note,
      date,
    };
  ...
  const options: Option[] = note.tags.map((value, index) => {
    return {
      label: value,
      value: value,
    };
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const res = await fetch(`${json_server_url}/json/notes/${note.id}`, {
      method: "PUT",
      headers: {
